...
public sealed class AiPromptCompiler : IAiPromptCompiler
{
    ...
    private const string PricingEmailIntegrityGuard = """
        ...
        - Las tablas HTML/Outlook pueden usar rowspan o celdas visualmente combinadas. Si una fila trae N-1 valores porque POD/POE está combinado con la fila anterior, hereda únicamente ese destino desde la última fila explícita y mantén las demás columnas en su posición. No interpretes el carrier como POD/POE ni un monto como carrier.
        - En una matriz POL, POD, CARRIER, 20', 40'/40HC, Free time, Effective Date, Expiry date, una fila corta como "Xingang WHL $5,958 $6,358 18 days 22-Sep 30-Sep" significa POL=Xingang, POD/POE heredado de la fila anterior, CARRIER=WHL, 20'=$5,958 y 40'/40HC=$6,358. El destino heredado sigue vigente hasta que aparezca uno nuevo de forma explícita.
        - Para una matriz con encabezados POL, POD, CARRIER, 20', 40'/40HC, Free time, Effective Date y Expiry date, cada fila lógica representa una fila comercial independiente. Un cambio de POL, carrier o monto inicia otra fila y jamás puede fusionarse con la anterior.
